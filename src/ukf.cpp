...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  if(!is_initialized_) {
    if(meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      x_ << meas_package.raw_measurements_[0], meas_package.raw_measurements_[1], 1.0, 0, 0;
      has_laser_data = true;
    }
  ...
  if((has_laser_data == true) && (has_radar_data == true)) {
    Prediction(dt);
  }
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    VectorXd point = Xsig_pred_.col(i);
    ...
    Zsig(2,i) = (p_x*v1 + p_y*v2 ) / (sqrt(p_x*p_x + p_y*p_y) + 0.00001);   //r_dot
  }
